var PubSubApp = (function () {
  ...
  const explodeSessions_ = (minifiedSessionsObject) => {
    return minifiedSessionsObject.ts.map((t) => {
      if (
        !minifiedSessionsObject.p.customerId ||
        !minifiedSessionsObject.p.deviceOrgUnitPath ||
        !minifiedSessionsObject.p.userOrgUnitPath
      ) {
  ...
  const goOnline = async (e) => {
    ...
    if (!vars.domain || !vars.userId) {
      // console.warn('attempted to publish without domain or user ID');
      let { domain, userId } = await refreshUserInfo()
        .then(({ domain, userId }) => {
          Object.assign(vars, { domain, userId });
          setLocalVars({ domain, userId });
          return { domain, userId };
    ...
    let privateIp = await SessionApp.getPrivateIP()
      .then((ip) => {
        vars.privateIp = ip;
        return ip;
      })
    ...
    let publicIp = await SessionApp.getPublicIP()
      .then((ip) => {
        vars.publicIp = ip;
        return ip;
      })
    ...
    if (resp.msgs.length) {
      ...
      if (success) {
        ...
        await new Promise((res, rej) =>
          chrome.storage.sync.remove(resp.keys, res)
        );
  ...
  const validateIntervalFunction = (checkRateInSeconds) => {
    ...
    const rate =
      ...
      appConfig.pubsub.pingRateInSeconds;
    ...
    activityIntervalId = setInterval(
      detectChromebookActivity,
      this.intervalInSeconds * 1000
    );
    addConnectivityListeners()
      .then(detectChromebookActivity)
      .catch((err) => {
  ...
  const sendPing = async () => {
    ...
    return window.navigator.onLine
      ...
      : cacheMessages([payload]);
  };

  const retrieveCachedMessages = async () => {
    let syncStorage = await new Promise((res, rej) =>
      ...
      (acc, k) => {
        if (pingExp.test(k)) {
          try {
            ...
            acc.msgs = acc.msgs.concat(
              explodeSessions_(JSON.parse(syncStorage[k]))
            );
            acc.keys.push(k);
          } catch (e) {
            console.error(e);
  ...
  const cacheMessages = async (payloads) => {
    ...
    let cacheValues = await new Promise((res, rej) =>
      chrome.storage.sync.get([key], res)
    );
    let msgs = (
      cacheValues[key] ? explodeSessions_(JSON.parse(cacheValues[key])) : []
    ).concat(payloads);
    let toStore = {};
  ...
  const publishTopic = async (payloads) => {
    ...
    return fetch(url, {
      ...
      .then(async (response) => {
        ...
        res.messageIds.forEach((mId) =>
          console.log(`Message with ID ${mId} has been published.`)
        );
        return true;
      })
      .catch(async (e) => {
  ...
  const detectChromebookActivity = () => {
    let detectionIntervalInSeconds = Number(
      this.intervalInSeconds || appConfig.pubsub.pingRateInSeconds
    );
    // console.log('detectionIntervalInSeconds', detectionIntervalInSeconds);
    return new Promise((res, rej) =>
      chrome.idle.queryState(detectionIntervalInSeconds, (state) => {
        if (state === "active") {
          sendPing()
            .then((r) => res(r))
            .catch((e) => rej(e));
  ...
  const checkForUpdatedParameters = async (apiResponse) => {
    ...
    if (
      "customerId" in apiResponse &&
      vars.customerId !== apiResponse.customerId
    ) {
      pingRequired = true;
    ...
    if (
      "deviceOrgUnitPath" in apiResponse &&
      vars.deviceOrgUnitPath !== apiResponse.deviceOrgUnitPath
    ) {
      pingRequired = true;
    ...
    if (
      "userOrgUnitPath" in apiResponse &&
      vars.userOrgUnitPath !== apiResponse.userOrgUnitPath
    ) {
      pingRequired = true;
    ...
    if (
      "interval" in apiResponse &&
      vars.pingInterval !== apiResponse.interval
    ) {
      pingRequired = true;
