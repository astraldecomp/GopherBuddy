...
chrome.runtime.onInstalled.addListener(function () {
  Analytics.sendEvent({
    category: "Extension",
    action: "Installed",
    label: appConfig.analytics.version,
  });
      ...
      if (deviceId) {
        ...
        if (window.navigator.onLine) {
          getUserProfile(deviceId, function (userInfo) {
            ...
            if (
              !vars.licenseTier ||
              !vars.licenseTimestamp ||
              new Date().getTime() - parseInt(vars.licenseTimestamp, 10) >
                appConfig.checkRateInSeconds.licenseRefresh * 1000
            ) {
              getLicenseTier(
                { userInfo: userInfo, deviceId: vars.deviceId },
                function (resp) {
                  switch (resp.tier) {
                    case "premium":
    ...
    if (!isManaged()) {
      ...
      if (!vars.licenseTier) {
        ...
        if (id === vars.deviceId) {
          ...
          var delayInMinutes =
            appConfig.checkRateInSeconds[vars.licenseTier] / 60;
          chrome.alarms.create(alarm.name, { delayInMinutes: delayInMinutes });
        }
      }
    }
...
function createAlarm(deviceId, alarmType) {
  ...
  if (!vars.licenseTier) {
    ...
    var delay =
      appConfig.checkRateInSeconds[vars.licenseTier] * 1000 +
      Math.round(Math.random() * 60, 0) * 1000; // TODO: PRODUCTION
...
function getUserProfile(deviceId, callback) {
  ...
  if (callback) {
    ...
    chrome.identity.getProfileUserInfo(function (userInfo) {
      getLicenseTier(
        { userInfo: userInfo, deviceId: deviceId },
        getLatestVersion,
        killSwitchCallback
      );
    });
...
function appendArguments(url, argumentsObject) {
  ...
  var fieldsToHash = ["ipAddress", "userEmail", "privateIp", "publicIp"];
...
function getDeviceInfoRenderPermission(params) {
  ...
  chrome.storage.local.get(["acl"], function (result) {
    ...
    if (
      acl &&
      new Date().getTime() - acl.timestamp <
        appConfig.syncCachePersistSeconds * 1000
    ) {
    ...
    chrome.identity.getProfileUserInfo(function (userInfo) {
      ...
      var baseUrl =
        appConfig.api.baseUrl +
        appConfig.api.version +
        appConfig.api.deviceInfoPermission;
        ...
        switch (responseCode) {
          ...
          case 304:
            ...
            chrome.storage.local.set(
              { acl: { access: result, timestamp: new Date().getTime() } },
              function (res) {}
            );
...
function getLatestVersion(params, successCallback, failureCallback) {
  ...
  var baseUrl =
    appConfig.api.baseUrl + appConfig.api.version + appConfig.api.deviceLookup;
    ...
    switch (responseCode) {
      ...
      case 304:
        ...
        if (responseObject.isCustomer && responseObject.domainIsAuthed) {
          setLocalVars({
            verboseAvailableVersion: responseObject.latestOsVersion,
          });
          if (vars.availableVersionSpoof) {
            console.log(
              "spoofed latest available version to " + vars.fakeAvailableVersion
            );
...
function extractPostBody(requestArguments) {
  ...
  var pickedProperties = _.pick(requestArguments, [
    "apiToken",
    "userDomain",
    "deviceId",
    "userId",
...
function compareVersion(params) {
  ...
  if (params.latestOs !== "Unknown") {
    ...
    if (
      vars.domainSettings.restrictChromeOsVersionUpdates &&
      _.has(vars.domainSettings, "chromeOsVersionMax")
    ) {
      // If configured to a max available version, set it here
      latestAvailableVersion = Math.min(
        latestAvailableVersion,
        vars.domainSettings.chromeOsVersionMax
      ); // Domain has major version cap set. Use the min of the two
    }

    if (
      vars.domainSettings.restrictChromeOsMinorVersion &&
      _.has(vars.domainSettings, "chromeOsMinorVersionMax")
    ) {
      latestAvailableMinorVersion = Math.min(
        latestAvailableMinorVersion,
        vars.domainSettings.chromeOsMinorVersionMax
      ); // Domain has a minor version cap set. Use the min of the two.
    }
  ...
  if (
    ...
    vars.domainSettings.allowChromeOsUpdateReminders
  ) {
    ...
    if (
      latestAvailableVersion - vars.deviceVersion > 0 && // The device version is behind the latest available version by some amount
      vars.domainSettings.popupThreshold && // There is a requirement to notify on one or more major versions versions behind
      latestAvailableVersion - vars.deviceVersion >=
        vars.domainSettings.popupThreshold
    ) {
      // The device version exceeds the notification threshold
      console.log("major version is behind - checking cap");
      showOldVersionPopup();
    }
    // Check minor version
    else if (
      vars.deviceVersion === latestAvailableVersion &&
      vars.domainSettings.minorVersionThreshold
    ) {
      ...
      var deviceMinorVersion = parseInt(
        vars.verboseDeviceVersion.split(".")[1],
        10
      );
      ...
      if (
        latestAvailableMinorVersion - deviceMinorVersion >=
        vars.domainSettings.minorVersionThreshold
      ) {
...
function showOldVersionPopup() {
  try {
    ...
    chrome.storage.local.get(["lastUpdateNotification"], function (data) {
      ...
      if (
        data.lastUpdateNotification + appConfig.popupAfterSeconds * 1000 >
        nowEpoch
      ) {
      ...
      if (vars.domainSettings.allowChromeOsUpdateReminders) {
        ...
        chrome.notifications.clear("versionNotice", function (wasCleared) {
          ...
          chrome.notifications.create(
            "versionNotice",
            appConfig.notices.outOfDate,
            function (notificationId) {
              chrome.storage.local.set(
                { lastUpdateNotification: now.getTime() },
                function (res) {}
              );
...
function getChromeOsVersion() {
  ...
  setLocalVars(
    { deviceVersion: localMajorVersion, verboseDeviceVersion: version },
    function (rtn) {
...
function triggerDisabledMode() {
  ...
  chrome.alarms.create(vars.deviceId + "|checkForDisabled", {
  ...
  chrome.storage.local.get(["deviceId"], function (data) {
    ...
    chrome.identity.getProfileUserInfo(function (userInfo) {
      return getLicenseTier(
        { userInfo: userInfo, deviceId: data.deviceId },
        function (response) {
...
chrome.runtime.onMessage.addListener(function (request, sender, respondWith) {
  ...
  if (request.req === "checkButtonRender") {
    getDeviceInfoRenderPermission(request).then(
      function (resp) {
